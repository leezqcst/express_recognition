...
parse.add_argument('--model', nargs='+',
...
dataset = {
    'train': h5Dataset(train_list),
    'val': h5Dataset(val_list)
}
...
dataloader = {
    ...
    'val': DataLoader(dataset['val'], batch_size=32, shuffle=False,
                      num_workers=opt.num_workers)
}

dimension = dataset['train'].dataset.size(1)

mynet = fcnet(dimension, opt.n_classes)
mynet.cuda()
...
for epoch in range(epoches):
    ...
    print('Loss: {:.6f}, Acc: {:.6f}, Time: {:.0f}s'.format(running_loss,
                                                            running_acc,
                                                            eplise_time))
    print('Validation')
    mynet.eval()
